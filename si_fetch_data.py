...
# https://stackoverflow.com/questions/63157046/python-http-request-exception-linetoolong
http.client._MAXLINE = 1000000000
...
def get_dcnm_version(ip):
    try:
        # putting curl timeout using --max-time
        cmd = 'curl -s -k https://{}/fm/fmrest/about/version --max-time 5'.format(
            ip)
        proc = run(cmd.split(), stdout=PIPE, stderr=STDOUT)
...
def start_port_forwarding(remote_ip):
    print("\n*******************************************\n")
    user = input(
        "Enter USER for DCNM remote ip {} [Recommended : root/admin for RHEL, sysadmin for OVA] : ".format(remote_ip))
    ...
    for local_port in range(33500, 33600):
        if is_port_free(local_port):
            ...
            cmd = 'ssh -4 -N -f -L localhost:{}:localhost:9200 {}@{}'.format(
                local_port, user, remote_ip)
        else:
...
def get_estimated_time_for_download(switchname, index_doc_count_list, cores):
    ...
    index_type_list = ['sanportrate', 'slowdraincounter', 'fc_flows', 'san_ect_baseline',
                       'san_fc_flows_rollup', 'fc_nvme_flows', 'san_nvme_ect_baseline', 'san_nvme_rollup']
    ...
    for index_type, doc_cnt in zip(index_type_list, index_doc_count_list):
        if doc_cnt and requires_estimation[index_type]:
            ...
            for _ in range(divisor):
                proc = multiprocessing.Process(target=mproc_estimate, args=(
                    LOCAL_PORT, index_type, switchname, match_all, return_list))
            ...
            report.append((index_type, int(1/download_rate),
                          doc_cnt, int((doc_cnt * download_rate)//60)))
    ...
    header = ['Index Type',
              'Download Rate(docs/sec)', 'Doc Count', 'Estimated Time(in minutes)']
...
def establish_es_client(local_port):
    ...
    try:
        localhost = '127.0.0.1'
        ES_CLIENT = Elasticsearch(hosts=[localhost], port=local_port, verify_certs=False,
                                  use_ssl=True, retry_on_timeout=True, max_retries=20, timeout=100)  # for Linux
        if ES_CLIENT.ping():
            print("Connected to {}:{}".format(localhost, local_port))
            pass
        else:
            ES_CLIENT = Elasticsearch(hosts=[localhost], port=local_port, verify_certs=False,
                                      use_ssl=False, retry_on_timeout=True, max_retries=20, timeout=100)  # for OVA/SE
            if ES_CLIENT.ping():
                print("Connected to {}:{}".format(localhost, local_port))
                pass
            else:
                print("Could not connect to ES {}:{}!".format(
...
def check_and_delete_residual_files():
    ...
    if residual_files:
        ...
        user_input = input(
            'The above file(s) will be deleted! Do you want to delete these files and continue? Y/N : ')
...
def get_ip_to_switchname_mapping():
    ...
    for item in data:
        if item['sysName'] not in switch_name_dict.keys():
            switch_name_dict[item['sysName']] = 'switch' + \
                str(len(switch_name_dict.keys()) + 1)
...
def rollup_index_sanity_check():
    ...
    for index in ES_CLIENT.indices.get('*'):
        if 'san_fc_flows_rollup_' in index:
            ...
            break
    resp = helpers.scan(ES_CLIENT, index=index_to_be_checked,
                        query=null_check_query)
    count = 0
    for _ in resp:
        count += 1
...
def get_switchname_from_swip(ip):
    ...
    resp = helpers.scan(ES_CLIENT, index='dcmdb-switch',
                        query=swip_switchname_query)
    ...
    if count == 0:
        ...
        logging.error(
            'Switch IP not found, please try again with different input.')
        exit_gracefully()
    elif count == 1:
...
def write_to_csv_file(filename, data):
    ...
    if not os.path.exists(filename):
        with open(filename, 'w', encoding='utf8', newline='') as output_file:
            ...
            # We do not want to write the header again
            fc = csv.DictWriter(
                output_file, fieldnames=data[0].keys(), extrasaction='ignore')
            fc.writeheader()
            fc.writerows(data)
    else:
...
def multiprocessing_func(pid, port, index, query, return_list, mapping_dict, concerned_keys, index_type, swip):
    ...
    if index_type == 'fc_flows':
        ...
        filename = filename.replace(
            'san_nvme_ect_baseline', 'fc_nvme_enrichdata')
    elif index_type == 'san_fc_flows_rollup':
        filename = filename.replace(
            'san_fc_flows_rollup', 'fc_scsi_1hr_rollup')
    ...
    if sub_total_doc:
        logging.error(
            "Number of relevant docs found in index %s : %s", index, sub_total_doc)
...
def fetch_pmdb_slowdraincounterdata(port, index_to_fetch, mapping_dict):
    ...
    if index_to_fetch['slowdraincounter']:
        ...
        with tqdm.tqdm(total=total_doc_cnt, desc=index, position=0) as pbar:
            resp = helpers.scan(ES_CLIENT, index=index,
                                query=match_all, raise_on_error=False)
            for doc in resp:
                doc = doc['_source']
                doc['swName'] = mapping_dict.get(doc['swName'], "unknown")
                data.append(doc)
...
def fetch_pmdb_sanportratedata(port, index_to_fetch, mapping_dict, pmdb_sanportratedata_concerned_keys):
    ...
    if index_to_fetch['sanportrate']:
        ...
        resp = helpers.scan(ES_CLIENT, index=index,
                            query=match_all_90_days, raise_on_error=False)
        try:
            with tqdm.tqdm(total=total_doc_cnt, desc=index, position=1) as pbar:
                for raw_doc in resp:
                    doc = {x: raw_doc['_source'][x]
                           for x in pmdb_sanportratedata_concerned_keys}
                    doc['switchName'] = mapping_dict.get(
                        doc['switchName'], "unknown")
                    data.append(doc)
...
def main(argv):
    ...
    INDEX_TO_FETCH = {"scsi": True, "nvme": True, "rawdata": False, "enrichdata": True,
                      "rollupdata": True, "sanportrate": True, "slowdraincounter": True}
    ...
    max_procs = 4
    # Shared resource between different processes
    manager = multiprocessing.Manager()
    ...
    try:
        opts, args = getopt.getopt(argv, "hd:s:e:i:p:v:i:t:l:n:v", [
                                   'help', 'dcnm=', 'swip=', 'exclude=', 'include=', 'port=', 'vsan=', 'init=', 'target=', 'lun=', 'nsid=', 'verbose', ])
        for opt, arg in opts:
            if opt in ("--help"):
                print('''
            ...
            if opt in ("--verbose"):
                ...
                # sets the global variable LOCAL_PORT
                start_port_forwarding(DCNM_REMOTE_IP)
                establish_es_client(LOCAL_PORT)
            elif opt in ("--swip"):
                SWIP = arg
                if DCNM_REMOTE_IP is None:
                    print(
                        'DCNM_REMOTE_IP is a required argument. Please provide a DCNM remote ip for which you want to extract the data.')
                    exit_gracefully()
                query_internals_for_fc_flows.append(
                    {"term": {"swIP": get_switchname_from_swip(SWIP)}})
                query_internals_for_san_ect_baseline.append(
                    {"term": {"swIP": SWIP}})
                ...
                if rollup_index_sanity_check():
                    query_internals_for_rollups.append(
                        {"terms": {"vsan": VSAN}})
            elif opt in ("--init"):
    ...
    if VERBOSE:
        ...
        print('query_internals_for_san_ect_baseline : ',
              query_internals_for_san_ect_baseline)
    ...
    for index in ES_CLIENT.indices.get('*'):
        if 'fc_flows_' + switchname.lower() in index:
            resp = ES_CLIENT.search(body=get_num_docs_query(
                query_internals_for_fc_flows), index=index)
            fc_flows_doc_count.append(resp['hits']['total'])
        elif 'san_ect_baseline_' + switchname.lower() in index:
            resp = ES_CLIENT.search(body=get_num_docs_query(
                query_internals_for_san_ect_baseline), index=index)
            san_ect_doc_count.append(resp['hits']['total'])
        elif 'san_fc_flows_rollup_' in index:
            resp = ES_CLIENT.search(body=get_num_docs_query(
                query_internals_for_rollups), index=index)
    ...
    index_doc_count_list = [pmdb_sanportratedata_doc_count, pmdb_slowdraincounterdata_doc_count,
                            sum(fc_flows_doc_count), sum(san_ect_doc_count), sum(
                                san_fc_flows_rollup_doc_count),
    ...
    est_time = get_estimated_time_for_download(
        switchname.lower(), index_doc_count_list, max_procs)
    if est_time > 60:
        print('\nApproximate time to fetch the required data : ~ {} minutes'.format(
            est_time//60))
    else:
    ...
    if pmdb_slowdraincounterdata:
        proc = multiprocessing.Process(target=fetch_pmdb_slowdraincounterdata, args=(
            LOCAL_PORT, INDEX_TO_FETCH, MAPPING_DICT))
        processes.append(proc)
        proc.start()
    if pmdb_sanportratedata:
        proc = multiprocessing.Process(target=fetch_pmdb_sanportratedata, args=(
            LOCAL_PORT, INDEX_TO_FETCH, MAPPING_DICT, pmdb_sanportratedata_concerned_keys))
        processes.append(proc)
        proc.start()
    ...
    if INDEX_TO_FETCH['scsi'] and INDEX_TO_FETCH['rawdata']:
        ...
        logging.error(
            "Searching for all indices starting with : %s", base_index)
        ...
        for pid, index in enumerate(arg_list):
            job = pool.apply_async(multiprocessing_func, args=(
                pid, LOCAL_PORT, index, match_itl_for_fc_flows, return_list, MAPPING_DICT, CONCERNED_KEYS, 'fc_flows', SWIP))
            jobs.append(job)
        ...
        os.system(
            " curl -s -X DELETE 'localhost:9200/_search/scroll/_all?pretty' > /dev/null ")
        end = time.time()

        if VERBOSE:
            print('\nFC FLOWS : Total docs = {} Time Taken = {} seconds \n'.format(
                sum(return_list), end-start))
    else:
        if VERBOSE:
    ...
    if INDEX_TO_FETCH['scsi'] and INDEX_TO_FETCH['enrichdata']:
        ...
        for pid, index in enumerate(arg_list):
            job = pool.apply_async(multiprocessing_func, args=(
                pid, LOCAL_PORT, index, match_itl_for_san_ect_baseline, return_list, MAPPING_DICT, CONCERNED_KEYS, 'san_ect_baseline', SWIP))
            jobs.append(job)
    ...
    if INDEX_TO_FETCH['scsi'] and INDEX_TO_FETCH['rollupdata']:
        ...
        for pid, index in enumerate(arg_list):
            job = pool.apply_async(multiprocessing_func, args=(
                pid, LOCAL_PORT, index, match_itl_for_rollups, return_list, MAPPING_DICT, CONCERNED_KEYS, 'san_fc_flows_rollup', SWIP))
            jobs.append(job)
    ...
    if INDEX_TO_FETCH['nvme'] and INDEX_TO_FETCH['rawdata']:
        ...
        for pid, index in enumerate(arg_list):
            job = pool.apply_async(multiprocessing_func, args=(
                pid, LOCAL_PORT, index, match_itl_for_fc_flows, return_list, MAPPING_DICT, CONCERNED_KEYS, 'fc_nvme_flows', SWIP))
            jobs.append(job)
    ...
    if INDEX_TO_FETCH['nvme'] and INDEX_TO_FETCH['enrichdata']:
        ...
        jobs = []
        for pid, index in enumerate(arg_list):
            job = pool.apply_async(multiprocessing_func, args=(pid, LOCAL_PORT, index, match_itl_for_san_ect_baseline,
                                   return_list, MAPPING_DICT, CONCERNED_KEYS, 'san_nvme_ect_baseline', SWIP))
            jobs.append(job)
    ...
    if INDEX_TO_FETCH['nvme'] and INDEX_TO_FETCH['rollupdata']:
        ...
        for pid, index in enumerate(arg_list):
            job = pool.apply_async(multiprocessing_func, args=(
                pid, LOCAL_PORT, index, match_itl_for_rollups, return_list, MAPPING_DICT, CONCERNED_KEYS, 'san_nvme_rollup', SWIP))
            jobs.append(job)
    ...
    print('Total time taken to fetch the required data : {} minutes'.format(
        (time.time()-job_start_time)//60))
    ...
    if not REMOTE:
        ...
        print(
            'All the data has been successfully generated at the specified remote location.')
    ...
    if VERBOSE:
        ...
        logging.error(
            'Switch names are mapped to abstract names in the csv files generated.')
        for key, value in MAPPING_DICT.items():
            logging.error('%s   -->   %s', key, value)
            print(key, ' '*(30-len(key)), '-->', ' '*10, value)

    print('\nTotal time taken to complete the task : {} minutes'.format(
        (time.time()-job_start_time)//60))
    stop_port_forwarding(LOCAL_PORT)
